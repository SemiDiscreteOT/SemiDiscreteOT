...
#ifndef wasserstein_solver_bfgs_h
#define wasserstein_solver_bfgs_h
 ...
 *
 * The behavior of the update step is selected at compile time via the
 * non-type template parameter `update_flag` of type UpdateMode.
 * Expected values include UpdateMode::TargetSupportOnly and
 * UpdateMode::TargetMeasureOnly.
 */

template <int spacedim, typename VectorType, UpdateMode update_flag>
class WassersteinSolverBFGS : public SolverBase<VectorType>
{
public:
  /**
   * Number type.
   ...
   * Constructor.
   */
  explicit WassersteinSolverBFGS(
    SolverControl        &residual_control,
    const std::function<Point<spacedim>(const Point<spacedim>&, const Vector<double>&)> &distance_function_exponential_map,
  ...
  void update(VectorType &x, const Number &alpha, const VectorType &p);
...
protected:
  ...
  const std::function<Point<spacedim>(const Point<spacedim>&, const Vector<double>&)>
...
template <int spacedim, typename VectorType, UpdateMode update_flag>
WassersteinSolverBFGS<spacedim, VectorType, update_flag>::AdditionalData::AdditionalData(
...
template <int spacedim, typename VectorType, UpdateMode update_flag>
WassersteinSolverBFGS<spacedim, VectorType, update_flag>::WassersteinSolverBFGS(
  SolverControl        &solver_control,
  const std::function<Point<spacedim>(const Point<spacedim>&, const Vector<double>&)> &distance_function_exponential_map_,
  const std::string &distance_name_,
  const AdditionalData &data)
  : SolverBase<VectorType>(solver_control)
  , additional_data(data)
  , distance_name(distance_name_)
  , distance_function_exponential_map(distance_function_exponential_map_)
{}
...
template <int spacedim, typename VectorType, UpdateMode update_flag>
boost::signals2::connection
WassersteinSolverBFGS<spacedim, VectorType, update_flag>::connect_line_search_slot(
  const std::function<
...
template <int spacedim, typename VectorType, UpdateMode update_flag>
boost::signals2::connection
WassersteinSolverBFGS<spacedim, VectorType, update_flag>::connect_preconditioner_slot(
  const std::function<void(VectorType                          &g,
                           const FiniteSizeHistory<VectorType> &s,
                           const FiniteSizeHistory<VectorType> &y)> &slot)
{
...
template <int spacedim, typename VectorType, UpdateMode update_flag>
void WassersteinSolverBFGS<spacedim, VectorType, update_flag>::update(VectorType &x, const Number &alpha, const VectorType &p)
{
  if constexpr (update_flag == UpdateMode::TargetSupportOnly)
  {
    ...
    {
      unsigned int n_barycenters = x.size()/spacedim;
      for (unsigned int i = 0; i < n_barycenters; ++i)
      {
        Point<spacedim> point;
        Vector<double> vector(spacedim);

        for (unsigned int j = 0; j < spacedim; ++j)
        {
          point[j] = x[i*spacedim + j];
          vector[j] = p[i*spacedim + j];
        }

        auto next_point = distance_function_exponential_map(point, vector);

        for (unsigned int j = 0; j < spacedim; ++j)
        {
          x[i*spacedim + j] += next_point[j];
        }
      }
    }
  }
  else if constexpr (update_flag == UpdateMode::TargetMeasureOnly)
  {
    ...
    // {
    //   x[i] = x[i]*std::exp(-alpha*p[i]*x[i]);
    //   total_mass += std::abs(x[i]);
    // }

    // if (total_mass > 0.0)
    // {
    //   x /= total_mass; // normalize measure
    // }
  }
}
...
template <int spacedim, typename VectorType, UpdateMode update_flag>
void
WassersteinSolverBFGS<spacedim, VectorType, update_flag>::solve(
      ...
      const auto default_line_min =
        [&](Number &f, VectorType &x, VectorType &g, const VectorType &p) {
          ...
          const auto line_func =
            ...
            x = x0;
            update(x, x_line, p); // x.add(x_line, p);
            f                   = compute(x, g);
          ...
          // loose line search:
          const auto res = WassersteinLineMinimization::line_search<Number>(
            line_func,
            f0,
            g0,
            WassersteinLineMinimization::poly_fit<Number>,
            a1,
            0.9,
            0.001,
          ...
          if (std::isnan(res.first))
          {
            ...
            {
              x = x0; // Revert to the previous step
              update(x, alpha, p); // Update x with p using x_line = 0.001
              f = compute(x, g);
              if (f < f_prev)
                break;
