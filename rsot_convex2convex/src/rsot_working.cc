...
void Convex2Convex<dim>::run_sot()
{
    ...
    if (solver_config.use_epsilon_scaling) {
        ...
        epsilon_sequence.reserve(solver_config.epsilon_scaling_steps);
        
        for (unsigned int i = 0; i < solver_config.epsilon_scaling_steps; ++i) {
            double scale_factor = std::pow(solver_config.epsilon_scaling_factor, 
                                         solver_config.epsilon_scaling_steps - 1 - i);
            epsilon_sequence.push_back(solver_config.regularization_param * scale_factor);
        }
    ...
    timer.stop();
    pcout << "\nSOT optimization completed in " << timer.wall_time() << " seconds" << std::endl;
}

template <int dim>
void Convex2Convex<dim>::run_target_multilevel(
        ...
        try {
            ...
            if(save_results_to_files) {
                ...
                if (Utilities::MPI::this_mpi_process(mpi_communicator) == 0) {
                    ...
                    conv_info << "Time taken: " << level_timer.wall_time() << " seconds\n";
                    pcout << "Level " << level_number << " results saved to " << level_output_dir << "/weights" << std::endl;
                }
            } else {
                pcout << "Level " << level_number << " completed in " << level_timer.wall_time() << " seconds" << std::endl;
            }
            ...
            pcout << "  Residual: " << exc.last_residual << std::endl;
...
void Convex2Convex<dim>::run_multilevel_sot()
{
    ...
    if (!multilevel_params.source_enabled && !multilevel_params.target_enabled) {
        pcout << "Error: Neither source nor target multilevel is enabled. Please enable at least one in parameters.prm" << std::endl;
        return;
    }
    ...
    if (multilevel_params.source_enabled) {
        pcout << "\nProcessing source mesh hierarchy..." << std::endl;
        ...
        for (size_t source_level = 0; source_level < source_mesh_files.size(); ++source_level) {
            ...
            if (source_level == 0) {
                ...
                try {
                    ...
                    // Run optimization using SotSolver
                    ...
                    pcout << "  Final number of iterations: " << sot_solver->get_last_iteration_count() << std::endl;
                    pcout << "  Final function value: " << sot_solver->get_last_functional_value() << std::endl;
    ...
    global_timer.stop();
    pcout << "\n============================================" << std::endl;
    pcout << "Multilevel computation completed!" << std::endl;
    pcout << "Total computation time: " << global_timer.wall_time() << " seconds" << std::endl;
    pcout << "Final results saved in: " << eps_dir << "/" << multilevel_dir << std::endl;
    pcout << "============================================" << std::endl;
}
...
void Convex2Convex<dim>::prepare_source_multilevel()
{
    ...
    try {
        ...
        pcout << "Level 1 (finest) vertices: " << multilevel_params.source_max_vertices << std::endl;
        pcout << "Coarsest level vertices: ~" << multilevel_params.source_min_vertices << std::endl;
