...
class PointCloudHierarchyManager:
    ...
    def kmeans_clustering(self, points, density, k):
        ...
        # Convert points to float32 (required by Faiss)
        points_float32 = np.ascontiguousarray(points, dtype=np.float32)

        # Get dimensionality of points
        d = points_float32.shape[1]
        ...
        try:
            ...
            if ngpus > 0:
                ...
                _, I = gpu_index.search(points_float32, 1)
                assignments = I.flatten()

                print("  Using GPU for KMeans clustering")
            else:
                # Fall back to CPU if no GPU is available
                kmeans = faiss.Kmeans(d, k, niter=300, verbose=False, seed=42, nredo=10)
                kmeans.train(points_float32)
                centroids = kmeans.centroids
                _, assignments = kmeans.index.search(points_float32, 1)
                assignments = assignments.flatten()
                print("  Using CPU for KMeans clustering (no GPU available)")
        except Exception as e:
            # Fall back to CPU in case of any GPU-related errors
            print(f"  GPU error: {e}")
