    ...
    if (Utilities::MPI::this_mpi_process(mpi_communicator) == 0) {
        ...
        if (target_params.use_custom_density) {
            ...
            if (target_params.density_file_format == "vtk") {
                if constexpr (dim==spacedim)
                {
                    DoFHandler<dim, spacedim> vtk_dof_handler_target;
                    Vector<double> vtk_field_target;
                    Triangulation<dim, spacedim> vtk_tria_target;
                    // Read VTK file only on rank 0, don't broadcast field
                    density_loaded = Utils::read_vtk_field<dim, spacedim>(target_params.density_file_path,
                    ...
                    if (density_loaded) {
                        ...
                        Functions::FEFieldFunction<dim, Vector<double>, spacedim> field_function(vtk_dof_handler_target, vtk_field_target);
...
template <int dim, int spacedim>
void SemiDiscreteOT<dim, spacedim>::save_results(const Vector<double>& potential, const std::string& filename)
{
