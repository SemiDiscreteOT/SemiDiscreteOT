...
int main(int argc, char* argv[]) {
    ...
    parallel::distributed::Triangulation<3> sphere_mesh(MPI_COMM_WORLD);
    ...
    // Only root process creates generators
    ...
    if (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0)
    {
        weights.resize(generators.size(), 1.0/generators.size());
    }
    ...
    power_diagram.output_vtu("power_diagram_3d");
